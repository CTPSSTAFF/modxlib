...
class ModxlibClass:
	...
	#
	all_time_periods = ['am', 'md', 'pm', 'nt']
	auto_modes = [ 'SOV', 'HOV' ]
	truck_modes = [ 'Heavy_Truck', 'Heavy_Truck_HazMat', 'Medium_Truck', 'Medium_Truck_HazMat', 'Light_Truck' ]
	nm_modes = [ 'Walk', 'Bike' ]
	transit_modes = [ 'DAT_Boat', 'DET_Boat', 'DAT_CR', 'DET_CR', 'DAT_LB', 'DET_LB', 'DAT_RT', 'DET_RT', 'WAT' ]
	all_modes = auto_modes + truck_modes + nm_modes + transit_modes
	...
	#
	def load_tts_as_np_arrays(self, tts, time_periods=None, mode_list=None):
	...
	#
	mode_to_metamode_mapping_table = {
		1:  'MBTA_Bus',
		2:  'MBTA_Bus',
		3:  'MBTA_Bus' ,
		4:  'Light_Rail',
	...
	#
	def mode_to_metamode(self, mode):
		retval = 'None'
		if mode in self.mode_to_metamode_mapping_table:
			return self.mode_to_metamode_mapping_table[mode]
		# end_if
		return retval
	# mode_to_metamode()
	...
	# Return value: N/A
	#
	def export_df_to_csv(self, dataframe, csv_fn, column_list=None):
		if column_list != None:
	...
	# Return value: N/A
	#
	def export_gdf_to_geojson(self, geo_dataframe, geojson_fn):
	...
	# Return value: N/A
	#
	def export_gdf_to_shapefile(self, geo_dataframe, shapefile_fn):
